...
public class MentorService:IMentorService
{
    ...
    public async Task<Response<string>> CreateMentorAsync(Mentor mentor)
    {
        try
        {
            var sql = $@"Insert into Mentor(fullName,age,salary)
                  values('{mentor.Fullname}', {mentor.Age}, {mentor.Salary})";
    ...
    public async Task<Response<string>> UpdateMentorAsync(Mentor mentor)
    {
        try
        {
            var sql = $@"Update MENTOR SET fullname='{mentor.Fullname}',age={mentor.Age},salary={mentor.Salary} 
               where id={mentor.Id}";
            var result=  await _context.Connection().ExecuteAsync(sql);
